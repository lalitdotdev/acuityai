...
export default function SummaryActions() {
    // get the summary context values and functions
    const {
        summaryPrompt,
        summaryIsGenerating,
        summaryModel,
    ...
    // use the copyToClipboard hook to copy the summary content to the clipboard
    const { isCopied, copyToClipboard } = useCopyToClipboard({
        timeout: 2000
    })

    // function to copy the summary content to the clipboard
    ...
    return (
        <div className="flex flex-row w-full justify-between items-center sticky top-0 z-20 bg-white pt-3.5 pb-2 px-3">
            <Select
                value={summaryModel.value} // value is the model value
                // setSummaryModel is a function that sets the model value
                onValueChange={(value) =>
